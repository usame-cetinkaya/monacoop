...
const suggestionRules: SuggestionRule[] = [
  ...
  // All Lowercase Text
  {
    pattern: (text: string) => {
      ...
      return (
        words.length > 1 &&
        words.every((word) => word === word.toLowerCase() && /[a-z]/.test(word))
      );
    },
  ...
  {
    pattern: /\s{2,}|\n\s*\n/,
    scripts: ["trim", "trimLines", "collapse"],
    confidence: 0.5,
...
export function analyzeClipboardAndSuggestScripts(
  text: string,
): ScriptSuggestion[] {
...
export function getScriptSuggestionTooltip(
