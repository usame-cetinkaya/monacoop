...
import * as TrimLines from "../scripts/TrimLines.js";
...
export const scripts: Record<string, BoopScript> = {
  ...
  md5: createScript(
    MD5,
    "MD5 Checksum",
    "Computes the MD5 checksum of your text",
    "🔐",
    "md5,hash,checksum",
    "crypto",
  ),
  ...
  rot13: createScript(
    Rot13,
    "ROT13",
    "Applies ROT13 cipher to your text",
    "🔄",
    "rot13,cipher,encode",
    "crypto",
  ),
  jwtDecode: createScript(
    JWTDecode,
    "JWT Decode",
    "Decodes JSON Web Tokens",
    "🗝️",
    "jwt,token,decode,json",
    "crypto",
  ),
  ...
  rgb2hex: createScript(
    rgb2hex,
    "RGB to Hex",
    "Converts RGB colors to hex",
    "🎨",
    "rgb,hex,color,convert",
    "colors",
  ),
...
export function getCategories(): string[] {
  const categories = new Set(
    Object.values(scripts).map((script) => script.metadata.category),
  );
...
// Execute a script
export function executeScript(key: string, state: State) {
